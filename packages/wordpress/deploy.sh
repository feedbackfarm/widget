...
# To Deploy, change version number in feedback-farm.php and readme.txt then run the script
...
SVNURL="http://plugins.svn.wordpress.org/feedback-farm" # Remote SVN repo on wordpress.org, with no trailing slash
SVNUSER="codemers" # your svn username
...
NEWVERSION1=`grep "^Stable tag" $PACKAGEDIR/readme.txt | awk -F' ' '{print $3}'`
...
cp -f ./feedback-farm.php $SVNPATH/trunk/
cp -f ./readme.txt $SVNPATH/trunk/
...
svn commit --username=$SVNUSER -m "Bump to version $NEWVERSION1"
